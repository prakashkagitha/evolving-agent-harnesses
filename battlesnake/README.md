...
# Evolving Agent Harnesses — BattleSnake (first result)

### Make 8 calls of a *small* model beat one call of a *frontier* model — by evolving the harness, not the model

> This was the project's **first** result. The repo now centers on **[Self-Harness on SWE-bench](../README.md)** (a model improving its *own* harness); this page preserves the original BattleSnake study in full.
...
- **Harness benefit is domain-dependent.** It needs a *deployable verifier* (game-engine win-rate here; unit-test execution for code) and a *beatable* frontier — which is exactly why the next study moved to SWE-bench.
...
Caveats, in the open: the headline metric is **BattleSnake ladder win-rate** (one game domain); win-rates are de-inflated point estimates with bootstrap CIs, not multi-seed means across domains. The full, unvarnished record — including the bugs we found and fixed — is in [`../results/RESULTS.md`](../results/RESULTS.md).
