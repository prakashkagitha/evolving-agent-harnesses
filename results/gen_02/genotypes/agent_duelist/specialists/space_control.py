...
def score(game_state):
    try:
        ...
        for move_name, (dx, dy) in moves.items():
            ...
            is_eating = new_head in food_positions
            ...
            # Strong food attraction (closest food)
            if food_positions:
                min_food_dist = min(abs(nx - fx) + abs(ny - fy) for fx, fy in food_positions)
                score_val += 20.0 - min_food_dist * 0.5
                if is_eating:
                    score_val += 30.0

            # Center tendency (reduced weight)
            center_x, center_y = width / 2.0, height / 2.0
            dist_to_center = abs(nx - center_x) + abs(ny - center_y)
            score_val -= dist_to_center * 0.05
